...
export default function Sidebar({
  ...
  const { getDisplayName, getUsername } = useAuth();
  ...
  const displayName = getDisplayName() || user?.displayName || 'User';
  ...
  return (
    <>
      ...
      `}>
        ...
        <div className="px-3 py-3 border-b border-border flex-shrink-0 flex flex-col gap-2">
          <button
            ...
            disabled={uploading}
            className="w-full py-2 px-3 rounded-lg text-xs font-semibold bg-accent text-bg-primary hover:bg-accent-soft transition-all duration-150 flex items-center justify-center gap-1.5 disabled:bg-text-hint disabled:text-text-muted disabled:cursor-not-allowed"
          >
            {uploading ? (
              <>
          ...
          {docs.length > 0 && (
            <div className="flex flex-col gap-1">
              <div className="flex items-center justify-between">
                <p className="text-[10px] font-semibold tracking-[0.1em] uppercase text-text-muted">Active PDFs</p>
                {docs.length > 1 && (
                  <button onClick={onClearDocs} className="text-[10px] text-text-muted hover:text-red-400 transition-colors">
                    Clear all
                  </button>
                )}
              </div>
              {docs.map((doc) => (
                <div key={doc.namespace} className="flex items-center gap-2 bg-accent-faint border border-accent/15 rounded-lg px-2.5 py-1.5 group">
                  <span className="text-accent flex-shrink-0"><PdfIcon /></span>
                  <span className="text-xs font-medium text-text-primary truncate flex-1">{doc.fileName}</span>
                  <button
        ...
        <div className="flex-1 overflow-y-auto py-2">
          <p className="px-4 pb-1.5 text-[10px] font-semibold tracking-[0.12em] uppercase text-text-muted">History</p>
          ...
          <ul className="flex flex-col gap-px">
            {chatList.map(chat => (
              <li
                ...
                className={`group flex items-center gap-2 px-3 py-2.5 mx-1 rounded-lg cursor-pointer transition-all duration-100 ${
        ...
        {user && (
          <div className="border-t border-border flex-shrink-0">
            {/* Profile row — click to open modal */}
            <button
              onClick={() => setShowProfile(true)}
              className="w-full flex items-center gap-2.5 px-3 py-3 hover:bg-bg-hover transition-colors group"
            >
              {user.photoURL ? (
                <img
                  src={user.photoURL}
            ...
            <div className="px-3 pb-3">
              <button
                onClick={onLogOut}
                className="w-full text-[11px] text-text-muted border border-border rounded-lg py-1.5 hover:border-border-light hover:text-text-primary transition-all duration-150"
              >
                Sign out
              </button>
            </div>
      ...
      {/* Profile modal — rendered outside sidebar so it overlays everything */}
